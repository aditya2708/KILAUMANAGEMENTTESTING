...
<html lang="en" dir="ltr">
    <head>
        ...
        <title>Admin | Berbagi Teknologi</title>
        ...
        <link rel="stylesheet" type="text/css" media="screen" href="{{ asset('bt/css/perfect-scrollbar.min.css') }}">
        <link rel="stylesheet" type="text/css" media="screen" href="{{ asset('bt/css/style.css') }}">
        <link defer="" rel="stylesheet" type="text/css" media="screen" href="{{ asset('bt/css/animate.css') }}">
        
        <link rel="stylesheet" href="{{ asset('bt/css/highlight.min.css') }}">
        
        <script src="{{ asset('bt/js/perfect-scrollbar.min.js') }}"></script>
        <script defer="" src="{{ asset('bt/js/popper.min.js') }}"></script>
        <script defer="" src="{{ asset('bt/js/tippy-bundle.umd.min.js') }}"></script>
        <script defer="" src="{{ asset('bt/js/sweetalert.min.js') }}"></script>
    </head>
    ...
    <body x-data="main" class="relative overflow-x-hidden font-nunito text-sm font-normal antialiased" :class="[ $store.app.sidebar ? 'toggle-sidebar' : '', $store.app.theme === 'dark' || $store.app.isDarkMode ?  'dark' : '', $store.app.menu, $store.app.layout,$store.app.rtlClass]">
        ...
        <div class="main-container min-h-screen text-black dark:text-white-dark" :class="[$store.app.navbar]">
                ...
                <nav x-data="sidebar" class="sidebar fixed bottom-0 top-0 z-50 h-full min-h-screen w-[260px] shadow-[5px_0_25px_0_rgba(94,92,154,0.1)] transition-all duration-300">
                    <div class="h-full bg-white dark:bg-[#0e1726]">
                        <div class="flex items-center justify-between px-4 py-3">
                            <a href="{{ url('berbagi-teknologi/home') }}" class="main-logo flex shrink-0 items-center">
                                <span class="align-middle text-2xl font-semibold ltr:ml-1.5 rtl:mr-1.5 dark:text-white-light lg:inline">Super SU</span>
                            </a>
            ...
            <div class="main-content flex min-h-screen flex-col">
                    ...
                    <div class="shadow-sm">
                        <div class="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-[#0e1726]">
                            <div class="horizontal-logo flex items-center justify-between ltr:mr-2 rtl:ml-2 lg:hidden">
                                <a href="{{ url('berbagi-teknologi/home') }}" class="main-logo flex shrink-0 items-center">
                                    <span class="hidden align-middle text-2xl font-semibold transition-all duration-300 ltr:ml-1.5 rtl:mr-1.5 dark:text-white-light md:inline">Super SU | Berbagi Teknologi</span>
                                </a>
                            ...
                            <div x-data="header" class="flex items-center space-x-1.5 ltr:ml-auto rtl:mr-auto rtl:space-x-reverse dark:text-[#d0d2d6] sm:flex-1 ltr:sm:ml-0 sm:rtl:mr-0 lg:space-x-2">
                                ...
                                <!--                <h4 class="text-lg">Log Activity</h4>-->
                                ...
                                <div class="dropdown" x-data="dropdown" @click.outside="open = false">
                                    ...
                                    <ul x-cloak="" x-show="open" x-transition="" x-transition.duration.300ms="" class="top-11 w-[300px] divide-y !py-0 text-dark ltr:-right-2 rtl:-left-2 dark:divide-white/10 dark:text-white-dark sm:w-[350px]">
                                        <li>
                                            <div class="flex items-center justify-between px-4 py-2 font-semibold hover:!bg-transparent">
                                                <h4 class="text-lg">Pemberitahuan</h4>
                                                <template x-if="notifications.length">
                                                    <span class="badge bg-primary/80" x-text="notifications.length + ' Baru'"></span>
                                                </template>
                                            </div>
                                        </li>
                                        <template x-for="notification in notifications">
                                        ...
                                        <template x-if="notifications.length">
                                            <li>
                                                <div class="p-4">
                                                    <button class="btn btn-primary btn-small block w-full" @click="toggle">Lihat Semua Pemberitahuan</button>
                                                </div>
                                            </li>
                                        </template>
                                        <template x-if="!notifications.length">
                                        ...
                                        <li>
                                            <div class="flex items-center px-4 py-4">
                                                <div class="flex-none">
                                                    ...
                                                    $cari = DB::table('karyawan')->select('gambar_identitas')->join('users','users.id_karyawan','=','karyawan.id_karyawan')->where('users.id', Auth::user()->id)->first();
                                                    @endphp
                                                    
                                                    <img class="h-10 w-10 rounded-md object-cover"  src="{{ asset('kilau/upload/' . ($cari->gambar_identitas ?? 'default.jpg')) }}" alt="image">
                        ...
                        <!-- horizontal menu -->
                        @include('bt-partials.sidebarHorizontal')
                    </div>
                </header>
                <!-- end header section -->
                ...
                <div class="mt-auto p-6 pt-0 text-center dark:text-white-dark ltr:sm:text-left rtl:sm:text-right" style="text-align: center">
                    © <span id="footer-year">{{date('Y')}}</span>. Berbagi Teknologi.
                </div>
                <!-- end footer section -->
            </div>
        </div>

        <script src="{{ asset('bt/js/alpine-collaspe.min.js') }}"></script>
        <script src="{{ asset('bt/js/alpine-persist.min.js') }}"></script>
        <script defer="" src="{{ asset('bt/js/alpine-ui.min.js') }}"></script>
        <script defer="" src="{{ asset('bt/js/alpine-focus.min.js') }}"></script>
        <script defer="" src="{{ asset('bt/js/alpine.min.js') }}"></script>
        <script src="{{ asset('bt/js/custom.js') }}"></script>
        
        <script defer="" src="{{ asset('bt/js/apexcharts.js') }}"></script>
        <script src="{{ asset('bt/js/simple-datatables.js') }}"></script>
        
        
        @extends('bt-js.apps')
        @extends('bt-js.company')
        @extends('bt-js.users')
        @extends('bt-js.akses')
        @stack('scripts')
        <script>
            document.addEventListener('alpine:init', () => {
                ...
                Alpine.data('header', () => ({
                    ...
                    getNotif() {
                        fetch("{{ url('berbagi-teknologi/notify') }}")
                    ...
                    async removeNotification(value = null) {
                        try {
                            let bodyData = value ? { id: value, baca: 0 } : { id : 0,  baca: 0 };
                            const response = await fetch("{{ url('berbagi-teknologi/hapusNotif') }}", {
                                method: 'POST',
                                headers: {
                            ...
                            if (!response.ok) {
                                throw new Error('Gagal menghapus pemberitahuan');
                            }
                    
                            const result = await response.json();
                            console.log('Pemberitahuan berhasil dihapus', result);
                            ...
                            if (value) {
                                ...
                                this.notifications = this.notifications.filter(d => d.read !== 0);
                            }
                        } catch (error) {
